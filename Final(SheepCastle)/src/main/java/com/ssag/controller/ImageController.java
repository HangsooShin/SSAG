...
public class ImageController {
  ...
  public HashMap<String, Object> analysisImage(@RequestParam("file") MultipartFile file,
      @AuthenticationPrincipal PrincipalDetails daDetails) throws Exception {
    HashMap<String, Object> map = new HashMap<String, Object>();
    System.out.println("??username" + daDetails.getUser().getUsername());
    // 로그인한 사용자의 username을 가져와서 id(int)를 얻는다.
    String username = daDetails.getUser().getUsername();
    ...
    if (!file.isEmpty()) { // 업로드할 파일이 존재할 경우에만
      ...
      // 경로
      String fileUrl = "C:/sources/images/trash/" + id + "/"; // 아이디 번호로 폴더 생성

      do {
        // 이미지 이름
        destinationFileName = RandomStringUtils.randomAlphanumeric(16) + "_" + id + ".png";
        // RandomStringUtils.randomAlphanumeric(16) + "_" + id + "." +
